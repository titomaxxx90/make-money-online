...
$('[promotionsSlider_JS]').each(function () {
    var container = $(this).find('.swiper-container')[0],
        prevBtn = $(this).find('.slider-prev-btn')[0],
        nextBtn = $(this).find('.slider-next-btn')[0],
        pagination = $(this).find('.slider-pagination')[0];
    ...
    $('[filtersSlider_JS]').each(function () {
        ...
        if ($(window).innerWidth() < 769) {
            ...
            var space = $(window).innerWidth();
            space = (space / 100) * 0.37356805044 * 5;
        }
    ...
    $('[ourWorksSlider_JS]').each(function () {
	    ...
	    $('[reviews2Slider_JS]').each(function () {
        var $container = $(this).find('.swiper-container'),
            $prevBtn = $(this).find('.slider-prev-btn'),
            $nextBtn = $(this).find('.slider-next-btn'),
            $pagination = $(this).find('.slider-pagination');
...
//             console.log(1);
    ...
    $('[quizSingleAnswer_JS]').on('click', function () {
		...
		let $next = $element.find('input').data('next')
		if($next !== undefined && $next !== null && $next !== ''){
            if($next == 0){
                $target = $('.quiz-question').length;
                console.log($target);
            }else{
                $target = $next;
            }
    ...
    $('[quizSocialSelect_JS]').on('click', function () {
        ...
        if (typeof $attrCheck !== 'undefined' && $attrCheck !== false) {
            ...
            $container.find('[quizSocialInput_JS]').attr('placeholder', 'Ваш номер телефона ');
        } else {
            $container.find('[quizSocialEmailInput_JS]').val('');
            $container.find('[quizSocialEmailInput_JS]').addClass('hidden');
            $container.find('[quizSocialSelect_JS].active').removeClass('active');
            $element.addClass('active');
            $container.find('[quizSocialInput_JS]').attr('placeholder', 'Ваш ' + $value);
        }
    });
...
// 		  var overItems = document.querySelectorAll('.over-item');
// 		  overItems.forEach(function(item) {
// 			item.style.animationPlayState = 'running';
// 		  });

// 		  var overMain = document.querySelector('.over--main');
// 		  overMain.style.zIndex = '-1';
// 		});
...
function playVideoInline(container) {
    var embedUrl = container.data('video-embed');
    if (!embedUrl) return;
    var iframe = $('<iframe>', {
        src: embedUrl,
        frameborder: 0,
...
$(document).ready(function() {
    ...
    $('.video-block__container').on('click', '[data-play-btn]', function(e) {
        e.preventDefault();
        var $container = $(this).closest('.video-block__container');
        if ($container.find('iframe').length === 0) {
            playVideoInline($container);
        }
    });
    
    // Вертикальные видео в слайдере
    $('.our-works__slide').on('click', '[data-play-btn]', function(e) {
        e.preventDefault();
        var $slide = $(this).closest('.our-works__slide');
        if ($slide.find('iframe').length === 0) {
            playVideoInline($slide);
        }
    });
});
